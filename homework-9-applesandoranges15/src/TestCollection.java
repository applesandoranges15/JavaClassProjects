...
public abstract class TestCollection<E> extends TestCase {
    ...
    protected final void testcol(Collection<E> l, String name, E... parts) {
        ...
        assertFalse(name + ".hasNext() is true, even after " + parts.length + " next() calls",
            it.hasNext());
        assertFalse(name + ".hasNext() is false, after only " + i + " next() calls",
            i < parts.length);
    }
    ...
    protected void setUp() {
        try {
            assert c.size() == 0;
            assertTrue(
                "Assertions not enabled.  Add -ea to VM Args Pane in Arguments tab of Run Configuration",
                false);
    ...
    /** Initialize c and e. The e array must have at least 10 elements. */
    ...
    public void test74() {
        ...
        assertTrue("after remove() after first remove(), hasNext() should still be true",
            it.hasNext());
        if (preserveOrder) {
            testcol(c, "{X9,8}", e[8]);
        }
    ...
    public void test75() {
        ...
        assertTrue("after remove() after second remove(), hasNext() should still be false",
            !it.hasNext());
        testcol(c, "{X1,X8}");
    }
